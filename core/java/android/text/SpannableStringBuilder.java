...
GraphicsOperations ,GetCharsDraw
{
	...
	private void moveGapTo(int where) {
		...
		if (debugG) System.out.println("mGapStart = " + mGapStart + " mGapLength = " + 
				mGapLength + " and where is= " + where);
	...
	private int change(boolean notify, int start, int end,
		...
		if (debugG) System.out.println("    mTextbefore = \"" + String.copyValueOf(mText) + "\"");

		/*if (debugG) System.out.println(
				"     mText      = \"" + String.copyValueOf(mText) +
				"\"\n     tb         = \"" + tb.toString() + 
				"\"\n     start      = " + start +
				"\n     end        = " + end +
				"\n     tbstart    = " + tbstart +
		...
		//madmack modification
		//TODO: fix gaps
		//ArShaper.shaper(mText, start, tbend-tbstart+1, "SpannableSB -- change()");	
		if (debugG) System.out.println(
				"     mText      = \"" + String.copyValueOf(mText) +
				"\"\n     tb         = \"" + tb.toString() + 
				"\"\n     start      = " + start +
		...
		//only if there is something to shape
		int tbdiff = tbend-tbstart;
		if (tbdiff>0) {
			if (tbdiff==1) { //if insertion only, try to reshape word.. must move to C library !
				String stringToShape = String.copyValueOf(mText);
				int indexStart = stringToShape.lastIndexOf(" ", start);
				int startShaping = (indexStart==-1)?0:indexStart;
				int indexEnd = stringToShape.indexOf(" ", start);
	...
	public SpannableStringBuilder replace(final int start, final int end,
		...
		if (end == start || tbstart == tbend) {
			...
			ArShaper.shaper(mText, 0, length(), "SSB -- in replace", true);
			//if (inserted==0 && origlen!=0)
			//	sendTextChange(recipients, start-1, origlen+1, inserted+1);
			//else 
	...
	public void drawText(Canvas c, int start, int end,
		...
		if (end <= mGapStart) {
			...
			c.drawText(mText, start + mGapLength, end - start, x, y, p); //drawing gap ? what ?!
		} else {
